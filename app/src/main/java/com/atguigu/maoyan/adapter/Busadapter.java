...
import com.atguigu.maoyan.Utils.Numberview;
...
public class Busadapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Busadapter(Context context, List<ShoppingCart> ShoppingCart, TextView tv_total_price, final CheckBox checkbox_all) {
        this.context = context;
        this.datas = ShoppingCart;
     ...
     * @param b
     */
    public void checkAll_none(boolean b) {
        if(datas != null && datas.size() >0){
            int number = 0;
            for (int i=0;i<datas.size();i++){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemview = View.inflate(context, R.layout.bus_item, null);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        ((ViewHolder)holder).checkbox.setChecked(cart.isCheck());
        ((ViewHolder)holder).tv_name.setText(cart.getTitle());
        ((ViewHolder)holder).tv_price.setText("￥" + cart.getPrice());
        ((ViewHolder)holder).number_add_sub_view.setValue(cart.getCount());

        //请求图片
        Glide.with(context).load(cart.getPic())
                .placeholder(R.drawable.background_icon01)
                .error(R.drawable.background_icon01).
                into(((ViewHolder)holder).iv_icon);

        ((ViewHolder)holder).number_add_sub_view.setOnButtonClickListener(new Numberview.OnButtonClickListener() {
            @Override
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private Numberview number_add_sub_view;
        ...
        public ViewHolder(View itemView) {
            ...
            number_add_sub_view = (Numberview) itemView.findViewById(R.id.number_add_sub_view);
